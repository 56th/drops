...
void LocalStokesCL::setupN_P2(double N_P2[30][30]) {
    for (size_t i = 0; i < 30; ++i) {
        ...
        for (size_t j = 0; j < 30; ++j) {
            auto js = j / 3; // scalar shape index
            auto jn = j - 3 * js; // nonzero vect component
            N_P2[i][j] = quad_2D(dot(qWind, qP2grad[js]) * dot(e_in, qP_row[jn]) * qP2Hat[is], q2Ddomain);
        }
    }
}
            ...
            for (int k=0; k<3; ++k) {
                for (int l=0; l<3; ++l) {
                    N_P1[3*i+k][3*j+l]= quad_2D(qP1Hat[j]
											   * dot(qP_row[k], qP_row[l])*qconvP1[i], q2Ddomain);
                }
            }
        }
    }
...
void StokesIFAccumulator_P2P1CL::update_global_system() {
    ...
    for(int i = 0; i < 4; ++i) {
        ...
        for(int j=0; j <4; ++j) {
            ...
            mM_ScalarP1_stab(ii, jj) += locM_ScalarP1_stab[i][j];
        }
    }
}
