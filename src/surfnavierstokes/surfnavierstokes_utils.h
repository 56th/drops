...
void RightComputeAverageIterations( std::stringstream& stream, double& average)
{
    int max = 0;
    double counter = 0.;
    while( !stream.eof()) {
...
void RightComputeVariationFromAverageIterations( std::stringstream& stream, double average, double& variation)
{
    int max = 0;
    double counter = 0.;
    while( !stream.eof()) {
        ...
        int value = std::stoi(stringvalue);
            ...
            if(std::abs(average - max) > variation) {
                variation += std::pow(average - value,2);
            }
    ...
    variation = std::sqrt(variation/counter);
}