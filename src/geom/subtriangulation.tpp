...
namespace DROPS {
    ...
    inline bool equalSigns(GridFunT const & f) {
        int sum = 0;
        for (auto const &val : f) sum += sign(val);
...
// This function is created to check if a sub-tetra has a face which is a cut face w.r.t. the interface
    inline bool
    on_cutBnd(const std::valarray<double> &ls, const std::valarray<byte> &ls_sign, const PrincipalLatticeCL &lattice,
              const PrincipalLatticeCL::TetraT &lattice_tet,
    ...
    TetraPartitionCL::make_partition(const PrincipalLatticeCL &lat, const std::valarray<double> &ls) {
        ...
        for (PrincipalLatticeCL::const_tetra_iterator lattice_tet = lat.tetra_begin(), lattice_end = lat.tetra_end();
            ...
            for (RefTetraPartitionCL::const_tetra_iterator it = cut.tetra_begin(), end = cut.tetra_end();
                 it != end; ++it)
        ...
        vertex_order_policy.sort_vertexes(vertexes_, edgecut.cut_vertex_container(), pos_vertex_begin_,
                                          neg_vertex_end_);
    }
    ...
    SPatchCL<Dim>::make_sub_facet(const RefPatchFacetT &ref_tri, const LatticeBodyT &lattice_tet,
                                  ...
                                  std::vector<Uint> &copied_vertexes,
                                  std::vector<RenumberVertexPairT> &zero_vertex_uses,
    ...
    SPatchCL<Dim>::make_patch(const LatticeT &lat, const std::valarray<double> &ls) {
        ...
        for (typename LatticeT::const_body_iterator lattice_tet = lat.body_begin(), lattice_end = lat.body_end();
            ...
            for (typename RefPatchT::const_facet_iterator it = cut.facet_begin(), end = cut.facet_end();
                 it != end; ++it) {
        ...
        for (std::vector<RenumberVertexPairT>::const_iterator it = zero_vertex_uses.begin();
             it != zero_vertex_uses.end(); ++it)
...
//***************************************************************************************************
    ...
    BndTriangPartitionCL::make_sub_triangle(const RefTrianglePartitionCL::TriangleT &ref_tri,
                                            const PrincipalLatticeCL::TetraT &lattice_tet,
                                            const double lset[4], Uint lattice_num_vertexes,
        ...
        for (Uint j = 0; j < 3; ++j) {
            ...
            if (loc_vert_num < 4)   // on the origianl vertices
                ...
                const Ubyte v0 = VertOfEdge(loc_vert_num - 4, 0),
                        v1 = VertOfEdge(loc_vert_num - 4, 1);
                tri[j] = lattice_num_vertexes + edgecut(lattice_tet[v0], lattice_tet[v1], lset[v0],
                                                        lset[v1]); // add the cut vertex if not find and return its number
            }
        }
        return tri;
    }
    ...
    BndTriangPartitionCL::make_partition2D(const PrincipalLatticeCL &lat, Uint face, const std::valarray<double> &ls) {
        ...
        for (PrincipalLatticeCL::const_tetra_iterator lattice_tet = lat.tetra_begin(), lattice_end = lat.tetra_end();
            ...
            if (OnCutBnd) {
                const RefTrianglePartitionCL cut(loc_ls_sign,
                                                 Ubyte(face));                          //casting from unsigned int to unsigned char?
                for (RefTrianglePartitionCL::const_triangle_iterator it = cut.triangle_begin(), end = cut.triangle_end();
                     it != end; ++it)
        ...
        if (loc_triangles.end() - loc_triangles.begin() != 0)
            std::copy(loc_triangles.begin(), loc_triangles.end(),
                      std::back_inserter(triangles_)); //insert the positive triangles

        //unsorted vertexes, other sorted policy could be added if needed in the future
        std::copy(lat.vertex_begin(), lat.vertex_end(),
                  std::back_inserter(vertexes_));             //insert all vertices in the principal lattice;
        VertexContT cut_vertexes = edgecut.cut_vertex_container();
        std::copy(cut_vertexes.begin(), cut_vertexes.end(),
                  std::back_inserter(vertexes_));         //insert all vertices generated by edge cut;
    }
