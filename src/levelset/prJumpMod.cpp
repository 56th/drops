...
void ErrorVel( const VelVecDescCL& v, const MultiGridCL& mg,
               const MatrixCL& M, const MatrixCL& A );
...
int main ( int argc, char** argv )
{
    try
    {
        ...
        ErrorVel( prob.v, MG, prob.M.Data.GetFinest(), prob.A.Data.GetFinest() );
               ...
               const MatrixCL& M, const MatrixCL& A )
{
    ...
    double l2_error = dot( M*error, error );
    double h1_error = dot( A*error, error ) + l2_error;
    l2_error = std::sqrt( l2_error );
    h1_error = std::sqrt( h1_error );
    std::cout << "||e_v||_L2 = " << l2_error << std::endl;
    std::cout << "||e_v||_H1 = " << h1_error << std::endl;
}
...
Point3DCL right_hand_side( const Point3DCL &p, double )
{
    ...
    Point3DCL result;
    result[0] = - std::exp(-r2)*(-2*y)*( 2*r2 - 5 );
    result[1] = - std::exp(-r2)*( 2*x)*( 2*r2 - 5 );
    result[2] =  0;
