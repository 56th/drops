...
void ExchangeCL::SysnumHandlerCL::CreateExchangeDataMPIType()
...
/// and edges must be the same.
{
    ...
    for (const_SendList2ProcIter it(SendList_.begin()), end(SendList_.end()); it!=end; ++it){
        ...
        ex_.ExList_.push_back(ExchangeDataCL(it->first));
        ex_.ExList_.back().CreateDataType(it->second.size(), Addr(blocklength), Addr(displacements));
    }
}

bool ExchangeCL::SysnumHandlerCL::Gather( DiST::TransferableCL& t, DiST::Helper::SendStreamCL& sendstream)
...
/// (Should be private!)
/// \param t reference to the simplex
/// \param sendstream buffer
{
    ...
    if ( t.Unknowns.Exist() && t.Unknowns.Exist(idx)){
        ...
        for (const_SendList2ProcIter it(SendList_.begin()), end(SendList_.end()); it!=end; ++it, ++pos){
            sendstream << ProcCL::MyRank();            // from processor
            sendstream << it->first;                   // to processor
            sendstream << findPos(it->second, dof);    // send position
            sendstream << dof;                         // local sysnum
            if (RowIdx_->IsExtended(dof)){
                sendstream << findPos(it->second, RowIdx_->GetXidx()[dof]);
                sendstream << RowIdx_->GetXidx()[dof];
            }
            else {
                sendstream <<  NoIdx;
                sendstream << NoIdx;
...
/// TransferSendOrder. (Should be private!)
/// \param t reference to the simplex
/// \param numData number of transferred data
/// \param sendstream buffer
{

    const Uint idx=RowIdx_->GetIdx();
    const Uint numUnkOnSimplex= RowIdx_->NumUnknownsSimplex(t);
    int  fromProc=-1, recvProc=-1;

    IdxT sendPos=NoIdx;
    IdxT localSysnum, remoteSysnum, sendPos_extended, remoteSysnum_extended;

    // Check if there are unknowns on the simplex and processor
    for ( size_t i=0; i<numData; ++i){
        recv >> fromProc;
        while ( fromProc!=ProcCL::Size()){
            recv >> recvProc >> sendPos >> remoteSysnum >> sendPos_extended >> remoteSysnum_extended;
            if ( t.Unknowns.Exist() && t.Unknowns.Exist(idx)){
                if ( recvProc==ProcCL::MyRank() && sendPos!=NoIdx){
                    sendPos*= numUnkOnSimplex;
                    localSysnum=t.Unknowns(idx);                        // local sysnum
                    // put these information into the lists
                    for (Uint j=0; j<numUnkOnSimplex; ++j){
                    ...
                    if (RowIdx_->IsExtended() && sendPos_extended!=NoIdx){
                        ...
                        sendPos_extended*= numUnkOnSimplex;
                        //remoteSysnum=buffer[5];
                        localSysnum=RowIdx_->GetXidx()[localSysnum];
                        for (Uint j=0; j<numUnkOnSimplex; ++j){
                            RecvSysnums_[fromProc][sendPos_extended+j]=localSysnum + j;
                            tmpMappingIdx_[fromProc][localSysnum+j]= remoteSysnum_extended +j;
                ...
                throw DROPSErrCL("ExchangeCL::HandlerScatterSysnums: Received data for unknowns on a simplex without unknowns.");
            }
            recv >> fromProc;
        }
    }
...
void ExchangeCL::SysnumHandlerCL::TransferSendOrder(bool CreateMap)
/// This function calls the DDD-Interface for vertices and edges to transfer
/// the send order. The interface of the InterfaceCL<VertexCL> and InterfaceCL<EdgeCL>
/// are used. The messages consists of a list of four numbers:
...
{
    ex_.numNeighs_= SendList_.size();
    maxNeighs_= ProcCL::GlobalMax((int)ex_.numNeighs_);     /// \todo Do we need maxNeighs_?

    DiST::PrioListT from, to; from.push_back( PrioMaster); to.push_back( PrioMaster); /// \todo PrioMaster correct for determining the ExchangeCL
    DiST::InterfaceCL::DimListT dims;
    if ( RowIdx_->NumUnknownsVertex()>0)
        dims.push_back(0);
    if ( RowIdx_->NumUnknownsEdge()>0)
        dims.push_back(1);
    if ( RowIdx_->NumUnknownsFace()>0)
        dims.push_back(2);
    if ( RowIdx_->NumUnknownsTetra()>0)
        dims.push_back(3);
    DiST::InterfaceCL interf( RowIdx_->TriangLevel(), from, to, dims);
    interf.PerformInterfaceComm( *this);
...
*/
    ex_.mapCreated_= CreateMap;
    // Static members are just used for the creation via DDD-Interface
    if (CreateMap){
        ex_.MappingIdx_ = tmpMappingIdx_;
        ex_.SysProc_.resize(tmpSysProc_.size());
        std::copy(tmpSysProc_.begin(), tmpSysProc_.end(), ex_.SysProc_.begin());
    } else {
        ex_.MappingIdx_.clear();
        ex_.SysProc_.clear();
    }
    ...
    for( RecvSysnumCT::const_iterator it(RecvSysnums_.begin()); it!=RecvSysnums_.end(); ++it){
        for (ExchangeDataCL::SysnumListCT::const_iterator sysit(it->second.begin()); sysit!=it->second.end(); ++sysit){
            if ( (*sysit)>=(int)RowIdx_->NumUnknowns()){
                ...
                throw DROPSErrCL("ExchangeCL::TransferSendOrder: Receive sequence has not been set correctly");
            }
        }
    }
#endif
...
void ExchangeCL::SysnumHandlerCL::CreateList(const MultiGridCL& mg, bool CreateMap, bool CreateAccDist)
...
{
    Uint numUnkVert= RowIdx_->NumUnknownsVertex(),
         numUnkEdge= RowIdx_->NumUnknownsEdge(),
         numUnk=     RowIdx_->NumUnknowns();
    ...
    if (numUnkVert>0)
        CollectSendSysNums(mg.GetTriangVertexBegin(), mg.GetTriangVertexEnd(), DistSysnums, numUnkVert);
    if (numUnkEdge>0)
        CollectSendSysNums(mg.GetTriangEdgeBegin(), mg.GetTriangEdgeEnd(), DistSysnums, numUnkEdge);
    ...
    for (SendList2ProcIter it(SendList_.begin()), end(SendList_.end()); it!=end; ++it){
      ...
      // Tell neighbor processors about the send order of the unknowns via a DiST interface
    TransferSendOrder(CreateMap);

      // Tell DataExchangeCL about the receive sequence
    for (CommListCT::iterator it(ex_.ExList_.begin()), end(ex_.ExList_.end()); it!=end; ++it){
        it->CreateSysnums( RecvSysnums_[it->GetProc()] );
    }

      // Create the local and distributed indices
    ex_.CreateIndices(RowIdx_, DistSysnums, CreateAccDist);
...
void ExchangeCL::CreateIndices(IdxDescCL *RowIdx, const VectorBaseCL<bool>& DistSysnums, bool forAccParDot)
{
    ...
    {
        Assert(mapCreated_,
               DROPSErrCL("ExchangeCL::CreateIndices: If inner products for accumulated Vectors should be performed, in ExchangeCL::CreateList the flag CreateMap must be set!"),
               DebugParallelNumC);
        AccDistIndex.clear();
...
///    handle accumulated vectors
{
    ...
    SysnumHandlerCL handler( *this, RowIdx);
    handler.CreateList( mg, CreateMap, CreateAccDist);
...
/// \brief Information about the size of sent data between proc
/** This function requires that all procs call this function*/
...
/// \param os Where to print the information
{
    ...
    else
    {
        ...
        {
            if (p!=Proc)                        // receive information from the other procs
...
/** Check equality by: number of local and distributed indices, size of handleable vectors,
    ...
    Then check included ExchangeDataCL's for equality by: proc number, number and position of receiving sysnums*/
