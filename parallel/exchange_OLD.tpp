...
void ExchangeCL::SysnumHandlerCL::CollectSendSysNums(const SimplexIterT& begin, const SimplexIterT& end,
                                    VectorBaseCL<bool>& DistSysnums, Uint stride)
/// Iterate over all simplices between begin and end and put the sysnum (if distributed
/// sysnums exist on the simplex) into the SendList_ and set flag in DistSysnum, that
/// the sysnum is distributed.
{
    ...
    for (SimplexIterT sit(begin); sit!=end; ++sit){                                 // for all simplices
        if (!sit->IsLocal() && sit->Unknowns.Exist() && sit->Unknowns.Exist(idx)){  // check for distributed sysnum
            ...
            for ( DiST::TransferableCL::ProcList_const_iterator procList(sit->GetProcListBegin()); procList!=sit->GetProcListEnd(); ++procList){
                const int procno=procList->first;
                /// \todo PrioMaster correct for ExchangeCL?
                if ( procno!=ProcCL::MyRank() && procList->second==PrioMaster ){    // if other processor stores unknowns
                    SendList_[procno].push_back(dof);
                    ...
                    if ( RowIdx_->IsExtended() && RowIdx_->GetXidx()[dof]!=NoIdx ){  // if the index is extended and this dof is extended, send these extensions, too
                        SendList_[procno].push_back(RowIdx_->GetXidx()[dof]);
...
template <typename T>
  IdxT ExchangeCL::SysnumHandlerCL::findPos(const std::vector<T>& a, const T& elem)
...
/** This procedure waits until all (I)sends from this proc into the wild
    ...
    \param vec     receive from other procs the values of the shared entries and add them
    \param tag     default -1: the tag used by this function. If tag=-1, then use the default tag=1001
    \param offset  default 0: For blocked vectors this offset is used to enter a special block
...
*/
{
    ...
    // Wait until all sends and receives are completed
    ProcCL::WaitAll(req);
...
double ExchangeCL::LocDot_(const VectorCL& x, const VectorCL& y, VectorCL* x_acc) const
{
    ...
    AccFromAllProc(*x_acc, SendRecvReq_);   // receive values from neighbors and sum them up
...
*  (x_acc) and one distributed vector (y) without    *
*  performing an global reduce. If y_acc not equal  *
*  zero, the accumulated form of y will be given     *
*  by return in y_acc.                              *
*****************************************************
...
double ExchangeCL::AccurLocDotOneAcc_(const VectorCL& x_acc, const VectorCL& y, VectorCL* y_acc) const
{
    ...
    AccFromAllProc(*y_acc, SendRecvReq_);   // receive values from neighbors and sum them up
...
*  accumulated form of x or y will be given by return*
*  in x_acc or y_acc.                               *
*****************************************************
...
double ExchangeCL::AccurLocDotBothAcc_(const VectorCL& x, const VectorCL& y, VectorCL* x_acc, VectorCL* y_acc) const
{
    ...
    AccFromAllProc(*x_acc, SendRecvReq_);                   // receive values from neighbors and sum them up
    AccFromAllProc(*y_acc, req_y, 0, &secondRecvBuf);       // receive values from neighbors and sum them up
...
double ExchangeCL::LocDot (const VectorCL& x, bool acc_x,
    ...
    /// \param[in]  useAccur should the more accurate but slower version be used
    ...
    /// \return              inner product of \a x and \a y without global reduce
{
    ...
    if (useAccur){
        ...
        {   // x and y are not accumulated, so accumulate one
            ...
            // x and y should be accumulated. That makes no sense
            throw DROPSErrCL("ExchangeCL::LocDot: It makes no sense to do LocDot to accumulate both vectors. Better use accurate version");
        }
...
/** To use this function in CreateList the parameter CreateMapIdx must be set on true!<br>
    ...
    \pre CreateList must be called with CreateMap=true
*/
{
    Assert (mapCreated_,
...
/** Parallel InnerProduct with two distributed (non-accumulated) vectors x and y. The result is
...
/** This function is slower as the function ExchangeCL::ParDotAcc, because a local
    copy of one of the input vectors have to be made
*/
...
/// \brief Parallel InnerProduct of two distributed vectors. Computes both accumulated vectors (maybe more accurate)
...
/// \brief Norm of a distributed non-accumulated vector with higher accuracy
...
/// \brief Norm of a distributed non-accumulated vector without global reduce (higher accuracy)
...
/// \param[in] x     non-accumulated vector.
/// \param[in] y_acc accumulated vector
...
double ExchangeCL::Norm_sq_Acc(VectorCL &r_acc, const VectorCL &r) const
    ...
    /// \param[in]  r local distributed vector
    /// \return       r^T * r
{
