...
class ExchangeDataCL : public ExchangeDataSendCL
{
  public:
    ...
    typedef std::vector<int> SysnumListCT;           ///< Sequence, where to store the received unknowns
...
/** This class is the main class for handling the exchange of numerical datas.<br>
    ...
    in two steps (sending data and receiving datas) between neighboring procs,
    an inner product of a distributed and a accumulated vector can be performed
    effective. Between sending and receiving the numerical data, the local
...
class ExchangeCL
{
  ...
  private:
    ...
    // Interface handler for creating the ExchangeCL
    class SysnumHandlerCL
    {
      private:
        ExchangeCL&    ex_;
        SendList2ProcT SendList_;
        RecvSysnumCT   RecvSysnums_;
        MappingIdxCT   tmpMappingIdx_;
        SysnumProcCT   tmpSysProc_;
        IdxDescCL*     RowIdx_;
        int            maxNeighs_;

        template <typename SimplexIterT>
        void CollectSendSysNums(const SimplexIterT& begin, const SimplexIterT& end, VectorBaseCL<bool>& DistSysnums, Uint stride);
        template <typename T>
      ...
      public:
        SysnumHandlerCL( ExchangeCL& ex, IdxDescCL* idx) : ex_(ex), RowIdx_(idx), maxNeighs_(-1) {}
  ...
  public:
    ...
    bool IsEqual(const ExchangeCL&, std::ostream*os=0) const;                           // for debugging, check if to ExchangeCL'es seems to be equal
};
